...
$(document).ready(function () {
	$(".subscribe-button, .unsubscribe-button").on("click", function () {
		...
		var action = $(this).hasClass("subscribe-button")
			? "subscribe"
			: "unsubscribe";
	...
	$("#commentaireContainer").on(
		...
		function () {
			...
			var dislikeCountElement = $(
				'.comment-dislike-count[data-comment-id="' + commentId + '"]'
			);
			...
			success: function (response) {
				if (response.success) {
					...
					var commentHtml =
						...
						"</p>" +
						"</div>" +
						'<p class="m-0 comment-text comment-content">' +
						response.commentaire +
						"</p>" +
						'<div class="d-flex btnsocialcommenter">' +
						'<div class="comment-like-section">' +
						'<a href="javascript:void(0);" class="comment-like-button" data-comment-id="' +
						response.commentId +
						'">' +
						...
						'<a href="javascript:void(0);" class="comment-dislike-button" data-comment-id="' +
						response.commentId +
						'">' +
						'<ion-icon name="thumbs-down-outline" class="comment-dislike-icon"></ion-icon>' +
						'<span class="comment-dislike-count" data-comment-id="' +
						response.commentId +
						'">' +
						response.dislikes +
						"</span>" +
					...
					$("#comment-dislike-count-" + response.commentId).text(
						commentDislikeCount
					);
					...
					var newCommentContent = $("#commentaireContainer")
